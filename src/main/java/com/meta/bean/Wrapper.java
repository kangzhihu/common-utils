...
import com.meta.bean.clazzz.ClassUtils;
import com.meta.exception.NoSuchPropertyException;
import com.meta.bean.clazzz.ClassGenerator;
import com.meta.bean.clazzz.ClassHelper;
import com.meta.bean.clazzz.ReflectUtils;
 ...
 * Time: 2016/12/27 22:30<br/>
 * Email:kangzhihu@163.com<br/>
 * Descriptions: <br/>
 * &nbsp;&nbsp;&nbsp;&nbsp;一个对象包装类，其包装装载了类的所有信息，极大的方便了获取对象类信息与对象的创建等,
 * 这个和自己写个ClassUtil工具类来获取类信息有啥好处？？
 */
public abstract class Wrapper {
    ...
    abstract public String[] getWritePropertyNames();
    ...
    abstract public Object gainNewInstance();
    /**
     * get property value.
     *
    ...
    private static Wrapper makeWrapper(Class<?> c){
        ...
        ClassLoader cl = ClassUtils.getCallerClassLoader(Wrapper.class);
        ...
        for( Map.Entry<String,Method> entry : ms.entrySet() )
        {
            ...
            if( ( matcher = ReflectUtils.GETTER_METHOD_DESC_PATTERN.matcher(md) ).matches() )
            {
                ...
                readPns.add(pn);
                if(tmpFields.contains(pn)){
                    fieldPns.add(pn);
                }
            }
            else if( ( matcher = ReflectUtils.IS_METHOD_DESC_PATTERN.matcher(md) ).matches() )
            {
                String pn = propertyName(matcher.group(1));
                c2.append(" if( $2.equals(\"").append(pn).append("\") ){ return ($w)w.").append(method.getName()).append("(); }");
                pts.put(pn, method.getReturnType());
        ...
        cc.addField("public static String[] readPns;"); // read property name array.
        cc.addField("public static String[] fieldPns;"); // field property name array.
        cc.addField("public static String[] writePns;");// write property name array.
        ...
        cc.addMethod("public String[] getReadPropertyNames(){ return readPns; }");
        cc.addMethod("public String[] getFieldPropertyNames(){ return fieldPns; }");
        cc.addMethod("public String[] getWritePropertyNames(){ return writePns; }");
        ...
        String newinstanceMethod = "public Object gainNewInstance(){return new " + name + "();}";
        if(!ClassHelper.hasConstructor(c)){//no default constructor
            newinstanceMethod = "public Object gainNewInstance(){return null;}";
        }
        cc.addMethod(newinstanceMethod);
        try
        {
            Class<?> wc = cc.toClass();
            // setup static field.
            wc.getField("pts").set(null, pts);
            wc.getField("readPns").set(null, readPns.toArray(new String[0]));
            wc.getField("fieldPns").set(null, fieldPns.toArray(new String[0]));
            wc.getField("writePns").set(null, writePns.toArray(new String[0]));
