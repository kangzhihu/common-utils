...
public class DateUtils {
    ...
    public static final <T extends Date> T addTime(T original, String type, int n){
        if(original==null) return null;
        long addTime = 0;
        if(StringUtils.equals(DateUtils.TYPE_DAY,type)) addTime = m_day * n;
        if(StringUtils.equals(DateUtils.TYPE_HOUR,type)) addTime = m_hour * n;
        if(StringUtils.equals(DateUtils.TYPE_MINUTE,type)) addTime = m_minute * n;
        if(StringUtils.equals(DateUtils.TYPE_SECOND,type)) addTime = m_second * n;
    ...
    public static Date getCurDateByWeek(Integer dayOfWeek){
        if(dayOfWeek == null || dayOfWeek<0 || dayOfWeek >7) return DateUtils.currentTime();
    ...
    public static List<Date> getCurWeekDateList(Date date){
        ...
        for(int i=1;i<=7;i++) dateList.add(DateUtils.addTime(date,TYPE_DAY, -curWeek + i ));
        return dateList;
    }
    public static Date getWeekLastDay(Date date){
        int curWeek = getWeek(date);
        return DateUtils.addTime(date, TYPE_DAY,7 - curWeek);
    }
